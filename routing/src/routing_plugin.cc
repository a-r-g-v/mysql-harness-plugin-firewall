...
static void start(const ConfigSection *section) {
  ...
  try {
    ...
    MySQLRouting r(config.mode, config.bind_address.port,
                  ...
                  config.error_logfile_path, config.query_logfile_path, config.learn_file_path,
                  config.learn_mode, config.logging_query, config.fill_error_message, 
                  config.blacklist_detection , config.blacklist_wordlist_path);
    try {
